...
class IA:
    ...
    def iaLvl2(self,grille):
        done = False
        for i in range(8):
            if not done and grille[i] != None and grille[i].joueur.nom != self.nom:
                if i == 0 or i == 1 or i == 3 or i == 4 or i == 6 or i == 7:
                    for j in range(len(self.main.cartes)):
                        if not done and self.main[j].gauche > grille[i].droite:
                            if grille[i+1] == None:
                                grille.poser(self.main[j],i+1)
                                done = True
                                break
                if not done and i == 1 or i == 2 or i == 4 or i == 5 or i == 7 or i == 8:
                    for j in range(len(self.main.cartes)):
                        if not done and self.main[j].droite > grille[i].gauche:
                            if grille[i-1] == None:
                                grille.poser(self.main[j],i-1)
                                done = True
                                break
                if not done and i == 0 or i == 1 or i == 2 or i == 3 or i == 4 or i == 5:
                    for j in range(len(self.main.cartes)):
                        if not done and self.main[j].haut > grille[i].bas:
                            if grille[i+3] == None:
                                grille.poser(self.main[j],i+3)
                                done = True
                                break
                if not done and i == 3 or i == 4 or i == 5 or i == 6 or i == 7 or i == 8:
                    for j in range(len(self.main.cartes)):
                        if not done and self.main[j].bas > grille[i].haut:
                            if grille[i-3] == None:
                                grille.poser(self.main[j],i-3)
                                done = True
                                break
